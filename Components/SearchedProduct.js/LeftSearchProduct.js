  ...
  return (
        <div className=' col-span-2 pt-20 flex justify-center' style={{backgroundColor:'rgba(241, 241, 241, 1)'}}>
          <div className=' h-full  flex flex-col gap-3 w-3/4 '>
            ...
            <div>
              <p style={{color:showCategory=== 1 ? red : 'black'}} className='font-semibold text-[17px]'>Fresh Farm</p>
              <div className=' flex justify-center w-full'>
                <div className=' w-3/4 flex flex-col gap-2'>
                {['All','Fruit', 'Vegetable', 'Seasonal'].map(((elt, indx) => <p onClick={()=> handleChange(1, indx, elt)} style={{color:(showCategory=== 1 && indx === currIndx.indx) ? red : 'black'}}  className=' cursor-pointer' key={indx}>{elt}</p>))}
                </div>
              </div>
            </div>
            <div>
              <p style={{color:showCategory === 2 ? red : 'black'}} className='font-semibold text-[17px]'>Starchy Food</p>
              <div className=' flex justify-center w-full'>
                <div className=' w-3/4 flex flex-col gap-2'>
                {['All','Root', 'Green Banana', 'Beans'].map(((elt, indx) => <p onClick={()=> handleChange(2, indx, elt)} style={{color:(showCategory=== 2 && indx === currIndx.indx) ? red : 'black'}} className='cursor-pointer' key={indx}>{elt}</p>))}
                </div>
              </div>
            </div>
          </div>

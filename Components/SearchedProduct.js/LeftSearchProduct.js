...
function LeftSearchProduct({currIndx, setCurrIndx, localShow, setLocalShow}) {
          ...
          <div className=' h-full  flex flex-col gap-3 w-3/4 '>
            ...
            <div>
              ...
              <div className=' flex justify-center w-full'>
                ...
                {['All Fresher','Fruits', 'Vegetable', 'Seasonal'].map(((elt, indx) => <p onClick={()=> handleChange(1, indx, elt)} style={{color:(currCategory=== 1 && indx === currIndx.indx) ? red : 'black'}}  className=' cursor-pointer' key={indx}>{indx===0 ? elt.split(' ')[0]:elt}</p>))}
                </div>
              </div>
            </div>
            <div>

...
function SimilarProduct({data, category, width=1000, similarRef, getCoords, handleLeave, description = true}) {
  ...
  return (
    <div className=' w-full overflow-hidden relative mt-5'>
      <div onMouseMove={getCoords} onMouseLeave={handleLeave}  ref={true? similarRef : null} className=' flex flex-row gap-4 similar'>
        {data.slice(0,similarprods).map((elt, indx) => <ProductCard description={description} key={indx} product={elt}/>)}

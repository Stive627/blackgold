...
function SimilarProduct({data, category, width=1000}) {
  ...
  const similarprods = width >600 ? 6 : 2
  return (
    <div className=' relative mt-5'>
      ...
      <button onClick={()=>bgRouter.push(`/filter?category=${category}`)}  className=' absolute right-0 top-1/2 bg-white border rounded-full p-2 border-gray-400 cursor-pointer bg-white'><ArrowForwardIos/></button>
    </div>
  )
}

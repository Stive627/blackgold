...
function ProductCard({product}) {
  ...
  const large = width > 650
  return (
    <div className=' flex-col border rounded-lg' style={{borderColor:'rgba(0, 0, 0, 0.25)', width:large?'340px':''}}>
      <Image style={{border:'2px', borderTopRightRadius:'7px', borderTopLeftRadius:'7px'}} alt={`image number${product._id}`} src={fetchLink(product.img.slice(6))} width={large?340:300} height={large?340:300}/>
      <div className=' p-2'>
        <p>{product.name}</p>
        <p>{product.unit}</p>
